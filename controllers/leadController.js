...
exports.NewMsClientsForCallList = async (req, res) => {
  try {
    ...
    // ✅ Fetch paginated leads
    const [newClientForCall] = await db.execute(
...
exports.mfClients = async (req, res) => {
  try {
    ...
    // Count
    const [[{ total }]] = await db.execute(`SELECT COUNT(*) as total ${base}`, params);
  ...
  try {
    ...
    if (status === "sip_done_converted") {
      ...
      const sql = `
        ...
        WHERE id = ? AND assigned_to = ?
      `;
      await db.execute(sql, [status, filePath, leadId, rmId]);
    ...
    const sql = `
      ...
      WHERE id = ? AND assigned_to = ?
    `;
    await db.execute(sql, [status, leadId, rmId]);
